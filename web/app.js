(() => {
  ...
  function subCard(s) {
    return `<button class="sub-card ${s.Skipped?'skipped':''} ${s.IsTrial?'trial':''}" type="button" data-edit-sub="${s.id}"${s.Skipped?' aria-label="'+esc(s.ServiceName)+', 이번 달 결제 건너뜀"':''}><span class="sub-content"><span class="sub-title">${esc(s.ServiceName)}</span><span class="sub-meta ${s.Skipped?'skip-status':''}">${s.Skipped?'이번 달 결제 건너뜀':s.IsTrial?`무료 체험 · ${s.BillingDate.replaceAll('-','.')}부터 결제`:`${esc(s.Category || '기타')} · ${cycle(s.BillingCycle)}`}</span></span><span class="sub-price">${money(s.amount,s.Currency)}<span>${s.IsTrial?'체험 후 결제':s.NextPayment.replaceAll('-','.')}</span></span></button>`;
  }
  ...
  function openSubForm(service=null, existing=null){
    ...
    ${edit?`<div class="edit-actions"><button class="button skip-action ${s.Skipped?'restore':''}" type="button" id="skipSub">${s.Skipped?'이번 결제 다시 포함':'이번 결제 건너뛰기'}</button><button class="button danger" type="button" id="cancelSub">구독 해지</button></div>`:''}`;
  ...
  function openSettings(){
    ...
    modalBody.innerHTML=`<div class="settings-tabs"><button class="active" type="button" data-tab="profile">기본 설정</button><button type="button" data-tab="account">계정</button><button type="button" data-tab="payments">결제수단</button><button type="button" data-tab="currencies">통화</button><button type="button" data-tab="notifications">알림</button><button type="button" data-tab="channels">연동</button><button type="button" data-tab="data">데이터 관리</button></div>
    ...
    <section class="settings-section" data-section="account"><div class="account-block"><h3>이메일 변경</h3><p class="help">다음 로그인부터 변경한 이메일을 사용해요.</p><form id="emailChangeForm"><div class="field-grid"><label class="field wide"><span>새 이메일</span><input name="email" type="email" maxlength="254" autocomplete="email" value="${esc(state.user.Email)}"></label><label class="field wide"><span>현재 비밀번호</span><input name="currentPassword" type="password" maxlength="72" autocomplete="current-password"></label></div><div class="form-error" aria-live="polite"></div><div class="form-actions"><button class="button primary" type="submit">이메일 변경</button></div></form></div><div class="account-block"><h3>비밀번호 변경</h3><p class="help">변경하면 다른 기기의 로그인 세션은 모두 종료돼요.</p><form id="passwordChangeForm"><div class="field-grid"><label class="field wide"><span>현재 비밀번호</span><input name="currentPassword" type="password" maxlength="72" autocomplete="current-password"></label><label class="field"><span>새 비밀번호</span><input name="newPassword" type="password" minlength="8" maxlength="72" autocomplete="new-password"></label><label class="field"><span>새 비밀번호 확인</span><input name="confirmPassword" type="password" minlength="8" maxlength="72" autocomplete="new-password"></label></div><div class="form-error" aria-live="polite"></div><div class="form-actions"><button class="button primary" type="submit">비밀번호 변경</button></div></form></div></section>`;
    bindSettings();
  }
  ...
  function bindSettings(){
    ...
    document.querySelector('#emailChangeForm').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.currentTarget),error=e.currentTarget.querySelector('.form-error'),email=String(f.get('email')).trim();error.textContent='';if(!email||!f.get('currentPassword')){error.textContent='이메일과 현재 비밀번호를 입력해 주세요.';return}try{await api('/api/account/email',{method:'PUT',body:{email,currentPassword:f.get('currentPassword')}});state.user.Email=email.toLowerCase();e.currentTarget.elements.currentPassword.value='';toast('로그인 이메일을 변경했어요.')}catch(err){error.textContent=err.message}});
    document.querySelector('#passwordChangeForm').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.currentTarget),error=e.currentTarget.querySelector('.form-error'),currentPassword=String(f.get('currentPassword')),newPassword=String(f.get('newPassword')),confirmPassword=String(f.get('confirmPassword'));error.textContent='';if(!currentPassword||!newPassword){error.textContent='현재 비밀번호와 새 비밀번호를 입력해 주세요.';return}if(newPassword!==confirmPassword){error.textContent='새 비밀번호 확인이 일치하지 않아요.';return}try{await api('/api/account/password',{method:'PUT',body:{currentPassword,newPassword}});e.currentTarget.reset();toast('비밀번호를 변경했어요. 다른 기기에서는 다시 로그인해 주세요.')}catch(err){error.textContent=err.message}});

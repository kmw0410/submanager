...
- 시스템·다크·라이트 테마, PWA 설치와 반응형 모바일 레이아웃 지원
...
설정의 **연동** 메뉴에서 Discord와 Telegram을 각각 켠 뒤 필요한 정보를 입력하고 테스트 알림을 보낼 수 있습니다. 비활성화한 연동은 입력 항목과 테스트 버튼이 숨겨지며, 저장된 정보는 다시 켤 때까지 보관됩니다. 같은 메뉴의 **PWA** 항목에서는 지원하는 브라우저에서 SubManager를 앱처럼 설치할 수 있습니다. PWA 설치는 일반적으로 HTTPS 환경에서 사용할 수 있으며, `localhost`는 예외입니다. **알림** 메뉴에서는 결제 예정 알림을 켜거나 끄고, 결제 며칠 전에 알릴지 0~30일 범위로 설정할 수 있습니다.

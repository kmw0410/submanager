...
- 미래 결제 예정 ICS 내보내기 지원 및 월간·3개월·1년 기간 선택
...
- 시스템·다크·라이트 테마, PWA 설치·기기 푸시 알림과 반응형 모바일 레이아웃 지원
...
Docker을 사용하여 간편하게 시작할 수 있습니다. `docker-compose.yml`을 내려받아 실행하세요.

```bash
docker compose up -d
```

관리자 계정이 없으면 시작할 때마다 48자리 최초 설정 키가 무작위로 생성되어 컨테이너 로그에 출력됩니다. 다음 명령으로 확인한 값을 최초 관리자 화면에 입력하세요.
...
설정의 **연동** 메뉴에서 Discord와 Telegram을 각각 켠 뒤 필요한 정보를 입력하고 테스트 알림을 보낼 수 있습니다. 비활성화한 연동은 입력 항목과 테스트 버튼이 숨겨지며, 저장된 정보는 다시 켤 때까지 보관됩니다. 같은 메뉴의 **PWA** 항목에서는 지원하는 브라우저에서 SubManager를 앱처럼 설치하고, **이 기기의 푸시 알림 켜기**를 눌러 결제 예정 알림을 받을 수 있습니다. 같은 곳의 **PWA 테스트**는 현재 기기로 테스트 결제 알림을 보냅니다. PWA 설치와 푸시 알림은 일반적으로 HTTPS 환경에서 사용할 수 있으며, `localhost`는 예외입니다. **알림** 메뉴에서는 결제 예정 알림을 켜거나 끄고, 결제 며칠 전에 알릴지 0~30일 범위로 설정할 수 있습니다.

같은 날 결제 예정인 항목은 하나의 알림으로 묶어서 전송되며, 이번 결제를 건너뛴 구독은 제외됩니다. Telegram은 MarkdownV2 형식으로 전송됩니다. 예약 알림 작업은 서버 시작 시 한 번 실행된 뒤 6시간마다 확인합니다. 켜 둔 Discord, Telegram, PWA 기기 푸시 채널로 각각 전송됩니다.

### 데이터 백업과 복원

설정의 **데이터 관리**에서 JSON 파일을 내보내거나 가져올 수 있습니다. 백업에는 구독, 결제수단, 통화, 결제 건너뛰기, 가격 이력, 활동 이력과 애플리케이션 설정이 포함됩니다. Discord Webhook, Telegram Bot Token·Chat ID와 PWA 푸시 구독 정보는 기본적으로 제외되며, 내보내기 전에 **알림 연동 정보 포함**을 선택해 함께 저장할 수 있습니다.
...
- 최초 관리자 설정 키는 계정이 없을 때 시작마다 새로 생성되어 컨테이너 로그에만 출력됩니다.

(() => {
  ...
  const money = (value, currency='KRW') => {try{return new Intl.NumberFormat('en-US',{style:'currency',currency,minimumFractionDigits:0,maximumFractionDigits:currency==='KRW'||currency==='JPY'?0:2}).format(Number(value||0))}catch{return `${currency} ${Number(value||0).toLocaleString('en-US')}`}};
  ...
  function subCard(s) {
    return `<button class="sub-card ${s.Skipped?'skipped':''} ${s.IsTrial?'trial':''}" type="button" data-edit-sub="${s.id}"><span class="sub-content"><span class="sub-title">${esc(s.ServiceName)}</span><span class="sub-meta">${s.Skipped?'이번 결제 건너뜀':s.IsTrial?`무료 체험 · ${s.BillingDate.replaceAll('-','.')}부터 결제`:`${esc(s.Category || '기타')} · ${cycle(s.BillingCycle)}`}</span></span><span class="sub-price">${money(s.amount,s.Currency)}<span>${s.IsTrial?'체험 후 결제':s.NextPayment.replaceAll('-','.')}</span></span></button>`;
  }
  ...
  function openSubForm(service=null, existing=null){
    ...
    modalBody.innerHTML=`<form id="subscriptionForm"><div class="field-grid">
      ...
      <label class="field"><span>금액 *</span><input name="amount" required type="number" min="0" step="1" inputmode="numeric" value="${esc(s.amount)}" placeholder="14900"></label>
    ...
    document.querySelector('#subscriptionForm').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.currentTarget);const body={serviceId:edit?s.ServiceID:(service?Number(service.ID):null),serviceName:f.get('serviceName'),icon:s.Icon||String(f.get('serviceName')).slice(0,1).toUpperCase(),color:s.Color||'#9AB8A8',amount:Number(f.get('amount')),currency:f.get('currency'),billingCycle:f.get('billingCycle'),billingDate:f.get('billingDate'),trialEndsAt:f.has('isTrial')?f.get('trialEndsAt'):'',paymentMethodId:Number(f.get('paymentMethodId')),category:f.get('category'),memo:f.get('memo')};try{await api(edit?`/api/subscriptions/${s.id}`:'/api/subscriptions',{method:edit?'PUT':'POST',body});closeModal();await refresh();toast(edit?'구독 정보를 바꿨어요.':'새 구독을 추가했어요.')}catch(err){document.querySelector('#formError').textContent=err.message}});
